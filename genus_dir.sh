...
# Some PDF may fail when moved because the can be still locked by the lowriter batch process (mv fail to stat)
...
for item in *.doc* ; do # for each file with .doc* in its name
	...
	genus_spp=$(echo $item | sed 's/\.doc.*//g')
	genus_spp=$(echo $genus_spp | sed 's/[ _]published.*//g') 
	...
	mv "$item" "$genus"/"$directory"/"$final_copy_name"
	...
	mv "$item_pdf" "$genus"/"$directory/$item_pdf"
